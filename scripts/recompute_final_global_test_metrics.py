#!/usr/bin/env python3
"""Re-evaluate the locked final checkpoints with dataset-level test metrics.

This script is intentionally separate from training and never modifies a
checkpoint or an original training CSV.  For every student checkpoint it
computes, over the complete test set:

* accuracy: total correct / total examples;
* NLL: summed cross-entropy / total examples;
* top-label ECE: one equal-width binning operation over all examples;
* teacher-student KL: mean KL(teacher || student) at temperature 1;
* output-Fisher mismatch: mean squared Frobenius distance between
  ``diag(p) - p p^T`` for the student and teacher;
* energy mismatch: mean squared difference of all unordered pairwise logit
  margins, matching ``energy_margin_loss`` in ``src/losses.py``.

The default alternating inventory is an explicit, validation-selected final
manifest.  Extra rho checkpoints are excluded unless
``--alternating-scope all-complete`` is requested; those rows are then marked
``sensitivity`` and are never silently relabelled as selected results.

Outputs are written atomically under ``results/global_evaluation_final`` and
the run can be resumed.  A teacher forward pass is cached once per
dataset/architecture pair and reused for all student diagnostics.
"""

from __future__ import annotations

import argparse
import csv
import os
import re
import sys
from dataclasses import dataclass
from pathlib import Path
from statistics import mean, stdev
from typing import Iterable, Optional

import torch
import torch.nn.functional as F
import yaml
from tqdm import tqdm


REPO_ROOT = Path(__file__).resolve().parents[1]
SRC_DIR = REPO_ROOT / "src"
if str(SRC_DIR) not in sys.path:
    sys.path.insert(0, str(SRC_DIR))

from data import get_image_test_loader  # noqa: E402
from losses import energy_margin_loss, output_fisher_loss  # noqa: E402
from models import make_model  # noqa: E402


DIAGNOSTIC_TEMPERATURE = 1.0
DEFAULT_SEEDS = (42, 43, 44, 45, 46)
DEFAULT_GROUPS = (
    "mnist",
    "cifar10_resnet",
    "cifar10_wrn",
    "cifar100_resnet",
    "cifar100_wrn",
)


@dataclass(frozen=True)
class GroupSpec:
    key: str
    label: str
    dataset: str
    architecture: str
    checkpoint_prefix: str
    student_config: str
    teacher_config: str
    teacher_checkpoint: str
    baseline_variants: tuple[str, ...]
    selected_alternating_variants: tuple[str, ...] = ()


GROUPS = {
    "mnist": GroupSpec(
        key="mnist",
        label="MNIST LeNet",
        dataset="mnist",
        architecture="LeNet",
        checkpoint_prefix="mnist_",
        student_config="configs/mnist_student_ce.yaml",
        teacher_config="configs/mnist_teacher.yaml",
        teacher_checkpoint="mnist_teacher.pt",
        baseline_variants=(
            "mnist_student_ce",
            "mnist_student_kd",
            "mnist_student_kd_fisher",
            "mnist_student_kd_energy",
            "mnist_student_kd_fisher_energy",
        ),
    ),
    "cifar10_resnet": GroupSpec(
        key="cifar10_resnet",
        label="CIFAR-10 ResNet",
        dataset="cifar10",
        architecture="ResNet-20",
        checkpoint_prefix="cifar_resnet20_",
        student_config="configs/cifar_resnet_student_ce.yaml",
        teacher_config="configs/cifar_resnet_teacher.yaml",
        teacher_checkpoint="cifar_resnet56_teacher.pt",
        baseline_variants=(
            "cifar_resnet20_student_ce",
            "cifar_resnet20_student_kd",
            "cifar_resnet20_student_kd_fisher",
            "cifar_resnet20_student_kd_energy",
            "cifar_resnet20_student_kd_fisher_energy",
        ),
        selected_alternating_variants=(
            "cifar_resnet20_kd_head_euclidean_alternating",
            "cifar_resnet20_kd_head_student_fisher_rho0p1_alternating",
            "cifar_resnet20_kd_head_teacher_fisher_rho1_alternating",
        ),
    ),
    "cifar10_wrn": GroupSpec(
        key="cifar10_wrn",
        label="CIFAR-10 WRN",
        dataset="cifar10",
        architecture="WRN-16-2",
        checkpoint_prefix="cifar_wrn16_2_",
        student_config="configs/cifar_wrn_student_ce.yaml",
        teacher_config="configs/cifar_wrn_teacher.yaml",
        teacher_checkpoint="cifar_wrn40_2_teacher.pt",
        baseline_variants=(
            "cifar_wrn16_2_student_ce",
            "cifar_wrn16_2_student_kd",
            "cifar_wrn16_2_student_kd_fisher",
            "cifar_wrn16_2_student_kd_energy",
            "cifar_wrn16_2_student_kd_fisher_energy",
        ),
        selected_alternating_variants=(
            "cifar_wrn16_2_kd_head_euclidean_alternating",
            "cifar_wrn16_2_kd_head_student_fisher_rho0p5_alternating",
            "cifar_wrn16_2_kd_head_teacher_fisher_rho0p5_alternating",
        ),
    ),
    "cifar100_resnet": GroupSpec(
        key="cifar100_resnet",
        label="CIFAR-100 ResNet",
        dataset="cifar100",
        architecture="ResNet-20",
        checkpoint_prefix="hifar_resnet20_",
        student_config="configs/hifar_resnet_student_ce.yaml",
        teacher_config="configs/hifar_resnet_teacher.yaml",
        teacher_checkpoint="hifar_resnet56_teacher.pt",
        baseline_variants=(
            "hifar_resnet20_student_ce",
            "hifar_resnet20_student_kd",
            "hifar_resnet20_student_kd_fisher",
            "hifar_resnet20_student_kd_energy",
            "hifar_resnet20_student_kd_fisher_energy",
        ),
        selected_alternating_variants=(
            "hifar_resnet20_kd_head_euclidean_alternating",
            "hifar_resnet20_kd_head_student_fisher_rho0p1_alternating",
            "hifar_resnet20_kd_head_teacher_fisher_rho0p1_alternating",
        ),
    ),
    "cifar100_wrn": GroupSpec(
        key="cifar100_wrn",
        label="CIFAR-100 WRN",
        dataset="cifar100",
        architecture="WRN-16-2",
        checkpoint_prefix="hifar_wrn16_2_",
        student_config="configs/hifar_wrn_student_ce.yaml",
        teacher_config="configs/hifar_wrn_teacher.yaml",
        teacher_checkpoint="hifar_wrn40_2_teacher.pt",
        baseline_variants=(
            "hifar_wrn16_2_student_ce",
            "hifar_wrn16_2_student_kd",
            "hifar_wrn16_2_student_kd_fisher",
            "hifar_wrn16_2_student_kd_energy",
            "hifar_wrn16_2_student_kd_fisher_energy",
        ),
        selected_alternating_variants=(
            "hifar_wrn16_2_kd_head_euclidean_alternating",
            "hifar_wrn16_2_kd_head_student_fisher_rho0p5_alternating",
            "hifar_wrn16_2_kd_head_teacher_fisher_rho0p5_alternating",
        ),
    ),
}


@dataclass(frozen=True)
class EvaluationTarget:
    group: GroupSpec
    family: str
    selection_status: str
    method: str
    variant: str
    seed: Optional[int]
    rho: Optional[float]
    checkpoint_path: Path
    config_path: Path
    original_metrics_path: Path


@dataclass
class TeacherCache:
    logits_batches: list[torch.Tensor]
    labels_batches: list[torch.Tensor]
    metrics: dict[str, float | int | None]


PER_CHECKPOINT_FIELDS = (
    "group",
    "dataset",
    "architecture",
    "family",
    "selection_status",
    "method",
    "variant",
    "seed",
    "rho",
    "n_test",
    "ece_bins",
    "diagnostic_temperature",
    "test_acc",
    "test_nll",
    "test_ece",
    "test_ts_kl",
    "test_fisher_mismatch",
    "test_energy_mismatch",
    "best_epoch",
    "old_batch_test_acc",
    "old_batch_test_nll",
    "old_batch_test_ece",
    "old_batch_test_ts_kl",
    "old_batch_test_fisher_mismatch",
    "old_batch_test_energy_mismatch",
    "test_acc_delta",
    "test_nll_delta",
    "test_ece_delta",
    "test_ts_kl_delta",
    "test_fisher_mismatch_delta",
    "test_energy_mismatch_delta",
    "checkpoint_path",
    "original_metrics_path",
    "source_config_path",
    "teacher_checkpoint_path",
    "checkpoint_size_bytes",
    "checkpoint_mtime_ns",
    "source_config_size_bytes",
    "source_config_mtime_ns",
    "teacher_checkpoint_size_bytes",
    "teacher_checkpoint_mtime_ns",
)


SUMMARY_FIELDS = (
    "group",
    "dataset",
    "architecture",
    "family",
    "selection_status",
    "method",
    "variant",
    "rho",
    "n_seeds",
    "ece_bins",
    "diagnostic_temperature",
    "test_acc_mean",
    "test_acc_std",
    "test_nll_mean",
    "test_nll_std",
    "test_ece_mean",
    "test_ece_std",
    "test_ts_kl_mean",
    "test_ts_kl_std",
    "test_fisher_mismatch_mean",
    "test_fisher_mismatch_std",
    "test_energy_mismatch_mean",
    "test_energy_mismatch_std",
)


def repository_path(path: str | Path) -> Path:
    path = Path(path)
    return path if path.is_absolute() else REPO_ROOT / path


def display_path(path: Path) -> str:
    try:
        return str(path.resolve().relative_to(REPO_ROOT.resolve()))
    except ValueError:
        return str(path.resolve())


def parse_seed(stem: str) -> tuple[str, Optional[int]]:
    match = re.fullmatch(r"(.+)_seed(\d+)", stem)
    if match is None:
        return stem, None
    return match.group(1), int(match.group(2))


def parse_rho(variant: str) -> Optional[float]:
    if "head_euclidean_alternating" in variant:
        return 0.0
    match = re.search(r"_rho([0-9]+(?:p[0-9]+)?)_alternating$", variant)
    if match is None:
        return None
    return float(match.group(1).replace("p", "."))


def method_label(variant: str, family: str) -> str:
    if family == "teacher":
        return "Teacher"
    if "head_euclidean_alternating" in variant:
        return "KD + Euclidean head [alternating]"
    if "head_student_fisher" in variant:
        return "KD + Student-Fisher head [alternating]"
    if "head_teacher_fisher" in variant:
        return "KD + Teacher-induced Fisher head [alternating]"
    if variant.endswith("_student_ce"):
        return "CE"
    if variant.endswith("_student_kd_fisher_energy"):
        return "KD + output Fisher + energy"
    if variant.endswith("_student_kd_fisher"):
        return "KD + output Fisher"
    if variant.endswith("_student_kd_energy"):
        return "KD + energy"
    if variant.endswith("_student_kd"):
        return "KD"
    return variant


def metrics_path_for_checkpoint(
    checkpoint: Path,
    checkpoint_root: Path,
    results_root: Path,
) -> Path:
    relative = checkpoint.relative_to(checkpoint_root)
    return (results_root / relative).with_suffix(".csv")


def add_seeded_variant(
    *,
    targets: list[EvaluationTarget],
    warnings: list[str],
    spec: GroupSpec,
    family: str,
    selection_status: str,
    variant: str,
    directory: Path,
    checkpoint_root: Path,
    results_root: Path,
    seeds: tuple[int, ...],
    allow_incomplete: bool,
) -> None:
    available = {
        seed: directory / f"{variant}_seed{seed}.pt"
        for seed in seeds
    }
    present = {seed for seed, path in available.items() if path.exists()}
    missing = sorted(set(seeds) - present)
    if missing:
        warnings.append(
            f"{spec.label}: {selection_status} variant {variant} "
            f"is missing seeds {missing}"
        )
        if not allow_incomplete:
            return

    for seed in sorted(present):
        checkpoint = available[seed]
        targets.append(
            EvaluationTarget(
                group=spec,
                family=family,
                selection_status=selection_status,
                method=method_label(variant, family),
                variant=variant,
                seed=seed,
                rho=parse_rho(variant) if family == "alternating" else None,
                checkpoint_path=checkpoint,
                config_path=repository_path(spec.student_config),
                original_metrics_path=metrics_path_for_checkpoint(
                    checkpoint, checkpoint_root, results_root
                ),
            )
        )


def discover_targets(
    specs: Iterable[GroupSpec],
    checkpoint_root: Path,
    results_root: Path,
    seeds: tuple[int, ...],
    include_baselines: bool,
    include_alternating: bool,
    include_teachers: bool,
    alternating_scope: str,
    allow_incomplete: bool,
) -> tuple[list[EvaluationTarget], list[str]]:
    targets: list[EvaluationTarget] = []
    warnings: list[str] = []

    for spec in specs:
        teacher_checkpoint = checkpoint_root / spec.teacher_checkpoint
        if include_teachers:
            if teacher_checkpoint.exists():
                targets.append(
                    EvaluationTarget(
                        group=spec,
                        family="teacher",
                        selection_status="reference",
                        method="Teacher",
                        variant=teacher_checkpoint.stem,
                        seed=None,
                        rho=None,
                        checkpoint_path=teacher_checkpoint,
                        config_path=repository_path(spec.teacher_config),
                        original_metrics_path=metrics_path_for_checkpoint(
                            teacher_checkpoint, checkpoint_root, results_root
                        ),
                    )
                )
            else:
                warnings.append(
                    f"{spec.label}: missing teacher checkpoint "
                    f"{display_path(teacher_checkpoint)}"
                )

        if include_baselines:
            baseline_dir = checkpoint_root / "seeds"
            for variant in spec.baseline_variants:
                add_seeded_variant(
                    targets=targets,
                    warnings=warnings,
                    spec=spec,
                    family="baseline",
                    selection_status="final",
                    variant=variant,
                    directory=baseline_dir,
                    checkpoint_root=checkpoint_root,
                    results_root=results_root,
                    seeds=seeds,
                    allow_incomplete=allow_incomplete,
                )

        if include_alternating and spec.selected_alternating_variants:
            alternating_dir = checkpoint_root / "head_metric" / "alternating" / "seeds"
            for variant in spec.selected_alternating_variants:
                add_seeded_variant(
                    targets=targets,
                    warnings=warnings,
                    spec=spec,
                    family="alternating",
                    selection_status="selected",
                    variant=variant,
                    directory=alternating_dir,
                    checkpoint_root=checkpoint_root,
                    results_root=results_root,
                    seeds=seeds,
                    allow_incomplete=allow_incomplete,
                )

            if alternating_scope == "all-complete":
                selected = set(spec.selected_alternating_variants)
                discovered: dict[str, set[int]] = {}
                pattern = f"{spec.checkpoint_prefix}*head_*_alternating_seed*.pt"
                for checkpoint in sorted(alternating_dir.glob(pattern)):
                    variant, seed = parse_seed(checkpoint.stem)
                    if variant in selected or seed is None or seed not in seeds:
                        continue
                    if not any(
                        token in variant
                        for token in (
                            "head_euclidean_alternating",
                            "head_student_fisher",
                            "head_teacher_fisher",
                        )
                    ):
                        continue
                    discovered.setdefault(variant, set()).add(seed)

                for variant in sorted(discovered):
                    missing = sorted(set(seeds) - discovered[variant])
                    if missing and not allow_incomplete:
                        warnings.append(
                            f"{spec.label}: sensitivity variant {variant} is "
                            f"missing seeds {missing}; skipped"
                        )
                        continue
                    add_seeded_variant(
                        targets=targets,
                        warnings=warnings,
                        spec=spec,
                        family="alternating",
                        selection_status="sensitivity",
                        variant=variant,
                        directory=alternating_dir,
                        checkpoint_root=checkpoint_root,
                        results_root=results_root,
                        seeds=seeds,
                        allow_incomplete=allow_incomplete,
                    )

    family_order = {"teacher": 0, "baseline": 1, "alternating": 2}
    targets.sort(
        key=lambda target: (
            DEFAULT_GROUPS.index(target.group.key),
            family_order[target.family],
            target.variant,
            -1 if target.seed is None else target.seed,
        )
    )
    return targets, warnings


def load_yaml(path: Path) -> dict:
    with path.open("r") as handle:
        return yaml.safe_load(handle)


def load_state_dict(path: Path, device: torch.device) -> dict:
    try:
        state = torch.load(path, map_location=device, weights_only=True)
    except TypeError:
        state = torch.load(path, map_location=device)

    if isinstance(state, dict):
        for key in ("state_dict", "model_state_dict"):
            if key in state and isinstance(state[key], dict):
                state = state[key]
                break
    if not isinstance(state, dict):
        raise TypeError(f"Unsupported checkpoint contents in {path}")

    if state and all(key.startswith("module.") for key in state):
        state = {key.removeprefix("module."): value for key, value in state.items()}
    return state


def global_expected_calibration_error(
    confidences: torch.Tensor,
    correctness: torch.Tensor,
    n_bins: int,
) -> float:
    """Compute equal-width top-label ECE once over the complete dataset."""
    if confidences.ndim != 1 or correctness.ndim != 1:
        raise ValueError("Confidences and correctness must be one-dimensional")
    if confidences.numel() != correctness.numel():
        raise ValueError("Confidences and correctness must have equal lengths")
    if confidences.numel() == 0:
        raise ValueError("Cannot compute ECE for an empty dataset")
    if n_bins <= 0:
        raise ValueError("n_bins must be positive")

    confidences = confidences.to(dtype=torch.float64, device="cpu")
    correctness = correctness.to(dtype=torch.float64, device="cpu")
    bin_edges = torch.linspace(0.0, 1.0, n_bins + 1, dtype=torch.float64)
    ece = torch.zeros((), dtype=torch.float64)

    for index in range(n_bins):
        in_bin = (confidences > bin_edges[index]) & (
            confidences <= bin_edges[index + 1]
        )
        count = int(in_bin.sum().item())
        if count == 0:
            continue
        bin_accuracy = correctness[in_bin].mean()
        bin_confidence = confidences[in_bin].mean()
        ece += (count / confidences.numel()) * torch.abs(
            bin_accuracy - bin_confidence
        )
    return float(ece.item())


def classification_metrics(
    *,
    correct_total: int,
    nll_total: float,
    example_total: int,
    confidences: list[torch.Tensor],
    correctness: list[torch.Tensor],
    n_bins: int,
) -> dict[str, float | int | None]:
    if example_total == 0:
        raise RuntimeError("Cannot evaluate an empty test loader")
    return {
        "n_test": example_total,
        "test_acc": correct_total / example_total,
        "test_nll": nll_total / example_total,
        "test_ece": global_expected_calibration_error(
            torch.cat(confidences), torch.cat(correctness), n_bins
        ),
        "test_ts_kl": None,
        "test_fisher_mismatch": None,
        "test_energy_mismatch": None,
    }


@torch.inference_mode()
def build_teacher_cache(
    *,
    spec: GroupSpec,
    loader,
    checkpoint_root: Path,
    device: torch.device,
    n_bins: int,
) -> TeacherCache:
    checkpoint = checkpoint_root / spec.teacher_checkpoint
    if not checkpoint.exists():
        raise FileNotFoundError(
            f"Teacher checkpoint is required for global diagnostics: {checkpoint}"
        )

    cfg = load_yaml(repository_path(spec.teacher_config))
    model = make_model(cfg["model"]).to(device)
    model.load_state_dict(load_state_dict(checkpoint, device), strict=True)
    model.eval()

    correct_total = 0
    nll_total = 0.0
    example_total = 0
    confidences: list[torch.Tensor] = []
    correctness: list[torch.Tensor] = []
    logits_batches: list[torch.Tensor] = []
    labels_batches: list[torch.Tensor] = []

    progress = tqdm(loader, desc=f"{spec.key}: cache teacher", leave=False)
    for images, labels in progress:
        images = images.to(device, non_blocking=True)
        labels = labels.to(device, non_blocking=True)
        logits = model(images)
        predictions = logits.argmax(dim=1)
        batch_correct = predictions.eq(labels)

        correct_total += int(batch_correct.sum().item())
        nll_total += float(F.cross_entropy(logits, labels, reduction="sum").item())
        example_total += labels.numel()
        confidences.append(F.softmax(logits, dim=1).max(dim=1).values.cpu())
        correctness.append(batch_correct.cpu())
        logits_batches.append(logits.cpu())
        labels_batches.append(labels.cpu())

    metrics = classification_metrics(
        correct_total=correct_total,
        nll_total=nll_total,
        example_total=example_total,
        confidences=confidences,
        correctness=correctness,
        n_bins=n_bins,
    )
    del model
    if device.type == "cuda":
        torch.cuda.empty_cache()
    return TeacherCache(logits_batches, labels_batches, metrics)


@torch.inference_mode()
def evaluate_student_checkpoint(
    *,
    target: EvaluationTarget,
    loader,
    teacher_cache: TeacherCache,
    device: torch.device,
    n_bins: int,
) -> dict[str, float | int | None]:
    cfg = load_yaml(target.config_path)
    model = make_model(cfg["model"]).to(device)
    model.load_state_dict(load_state_dict(target.checkpoint_path, device), strict=True)
    model.eval()

    correct_total = 0
    nll_total = 0.0
    kl_total = 0.0
    fisher_total = 0.0
    energy_total = 0.0
    example_total = 0
    confidences: list[torch.Tensor] = []
    correctness: list[torch.Tensor] = []

    progress = tqdm(
        loader,
        desc=f"{target.group.key}: {target.variant} seed={target.seed}",
        leave=False,
    )
    for batch_index, (images, labels) in enumerate(progress):
        if batch_index >= len(teacher_cache.logits_batches):
            raise RuntimeError("Test loader produced more batches than teacher cache")

        cached_labels = teacher_cache.labels_batches[batch_index]
        if not torch.equal(labels.cpu(), cached_labels):
            raise RuntimeError(
                "Test example order differs from the cached teacher pass; "
                "diagnostics would be invalid"
            )

        images = images.to(device, non_blocking=True)
        labels = labels.to(device, non_blocking=True)
        teacher_logits = teacher_cache.logits_batches[batch_index].to(
            device, non_blocking=True
        )
        student_logits = model(images)
        if student_logits.shape != teacher_logits.shape:
            raise RuntimeError(
                f"Student/teacher logit shapes differ: {student_logits.shape} vs "
                f"{teacher_logits.shape}"
            )

        predictions = student_logits.argmax(dim=1)
        batch_correct = predictions.eq(labels)
        batch_size = labels.numel()

        correct_total += int(batch_correct.sum().item())
        nll_total += float(
            F.cross_entropy(student_logits, labels, reduction="sum").item()
        )
        example_total += batch_size

        log_p_student = F.log_softmax(
            student_logits / DIAGNOSTIC_TEMPERATURE, dim=1
        )
        p_teacher = F.softmax(teacher_logits / DIAGNOSTIC_TEMPERATURE, dim=1)
        kl_total += float(
            F.kl_div(log_p_student, p_teacher, reduction="sum").item()
        )
        fisher_total += float(
            output_fisher_loss(student_logits, teacher_logits).item()
        ) * batch_size
        energy_total += float(
            energy_margin_loss(student_logits, teacher_logits).item()
        ) * batch_size

        probabilities = F.softmax(student_logits, dim=1)
        confidences.append(probabilities.max(dim=1).values.cpu())
        correctness.append(batch_correct.cpu())

    if len(teacher_cache.logits_batches) != len(teacher_cache.labels_batches):
        raise RuntimeError("Corrupt teacher cache: logits/labels batch counts differ")
    if example_total != int(teacher_cache.metrics["n_test"]):
        raise RuntimeError(
            f"Student evaluated {example_total} examples but teacher cache contains "
            f"{teacher_cache.metrics['n_test']}"
        )

    metrics = classification_metrics(
        correct_total=correct_total,
        nll_total=nll_total,
        example_total=example_total,
        confidences=confidences,
        correctness=correctness,
        n_bins=n_bins,
    )
    metrics.update(
        {
            "test_ts_kl": kl_total / example_total,
            "test_fisher_mismatch": fisher_total / example_total,
            "test_energy_mismatch": energy_total / example_total,
        }
    )
    del model
    if device.type == "cuda":
        torch.cuda.empty_cache()
    return metrics


def read_original_test_row(path: Path) -> dict[str, str]:
    if not path.exists():
        return {}
    with path.open("r", newline="") as handle:
        test_rows = [
            row for row in csv.DictReader(handle) if row.get("phase") == "test"
        ]
    return test_rows[-1] if test_rows else {}


def optional_float(value: object) -> Optional[float]:
    if value in (None, ""):
        return None
    return float(value)


def optional_metric(value: object) -> object:
    return "" if value is None else value


def difference(
    new_value: Optional[float], old_value: Optional[float]
) -> object:
    if new_value is None or old_value is None:
        return ""
    return new_value - old_value


def make_result_row(
    *,
    target: EvaluationTarget,
    metrics: dict[str, float | int | None],
    n_bins: int,
    checkpoint_root: Path,
) -> dict[str, object]:
    old = read_original_test_row(target.original_metrics_path)
    old_metrics = {
        "test_acc": optional_float(old.get("test_acc")),
        "test_nll": optional_float(old.get("test_nll")),
        "test_ece": optional_float(old.get("test_ece")),
        "test_ts_kl": optional_float(old.get("test_ts_kl")),
        "test_fisher_mismatch": optional_float(old.get("test_fisher_mismatch")),
        "test_energy_mismatch": optional_float(old.get("test_energy_mismatch")),
    }
    # CE training did not have a teacher forward pass; historical diagnostic
    # zeros in those CSVs mean "not measured", not a genuine zero mismatch.
    if target.family == "teacher" or target.variant.endswith("_student_ce"):
        for key in (
            "test_ts_kl",
            "test_fisher_mismatch",
            "test_energy_mismatch",
        ):
            old_metrics[key] = None

    checkpoint_stat = target.checkpoint_path.stat()
    config_stat = target.config_path.stat()
    teacher_checkpoint = checkpoint_root / target.group.teacher_checkpoint
    teacher_stat = teacher_checkpoint.stat()

    row: dict[str, object] = {
        "group": target.group.key,
        "dataset": target.group.dataset,
        "architecture": target.group.architecture,
        "family": target.family,
        "selection_status": target.selection_status,
        "method": target.method,
        "variant": target.variant,
        "seed": "" if target.seed is None else target.seed,
        "rho": "" if target.rho is None else target.rho,
        "n_test": metrics["n_test"],
        "ece_bins": n_bins,
        "diagnostic_temperature": DIAGNOSTIC_TEMPERATURE,
        "best_epoch": old.get("best_epoch", ""),
        "checkpoint_path": display_path(target.checkpoint_path),
        "original_metrics_path": display_path(target.original_metrics_path),
        "source_config_path": display_path(target.config_path),
        "teacher_checkpoint_path": display_path(teacher_checkpoint),
        "checkpoint_size_bytes": checkpoint_stat.st_size,
        "checkpoint_mtime_ns": checkpoint_stat.st_mtime_ns,
        "source_config_size_bytes": config_stat.st_size,
        "source_config_mtime_ns": config_stat.st_mtime_ns,
        "teacher_checkpoint_size_bytes": teacher_stat.st_size,
        "teacher_checkpoint_mtime_ns": teacher_stat.st_mtime_ns,
    }

    for key in (
        "test_acc",
        "test_nll",
        "test_ece",
        "test_ts_kl",
        "test_fisher_mismatch",
        "test_energy_mismatch",
    ):
        new_value = optional_float(metrics.get(key))
        old_value = old_metrics[key]
        row[key] = optional_metric(new_value)
        row[f"old_batch_{key}"] = optional_metric(old_value)
        row[f"{key}_delta"] = difference(new_value, old_value)
    return row


def read_csv(path: Path) -> list[dict[str, str]]:
    if not path.exists():
        return []
    with path.open("r", newline="") as handle:
        return list(csv.DictReader(handle))


def write_csv_atomic(
    path: Path, fieldnames: Iterable[str], rows: Iterable[dict]
) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    temporary = path.with_suffix(path.suffix + ".tmp")
    with temporary.open("w", newline="") as handle:
        writer = csv.DictWriter(
            handle,
            fieldnames=list(fieldnames),
            extrasaction="ignore",
        )
        writer.writeheader()
        writer.writerows(rows)
    os.replace(temporary, path)


def result_is_current(
    *,
    row: dict[str, str],
    target: EvaluationTarget,
    checkpoint_root: Path,
    n_bins: int,
) -> bool:
    checkpoint_stat = target.checkpoint_path.stat()
    config_stat = target.config_path.stat()
    teacher_checkpoint = checkpoint_root / target.group.teacher_checkpoint
    if not teacher_checkpoint.exists():
        return False
    teacher_stat = teacher_checkpoint.stat()

    try:
        metadata_matches = (
            row.get("selection_status") == target.selection_status
            and int(row.get("ece_bins", "")) == n_bins
            and float(row.get("diagnostic_temperature", ""))
            == DIAGNOSTIC_TEMPERATURE
            and int(row.get("checkpoint_size_bytes", ""))
            == checkpoint_stat.st_size
            and int(row.get("checkpoint_mtime_ns", ""))
            == checkpoint_stat.st_mtime_ns
            and int(row.get("source_config_size_bytes", "")) == config_stat.st_size
            and int(row.get("source_config_mtime_ns", ""))
            == config_stat.st_mtime_ns
            and int(row.get("teacher_checkpoint_size_bytes", ""))
            == teacher_stat.st_size
            and int(row.get("teacher_checkpoint_mtime_ns", ""))
            == teacher_stat.st_mtime_ns
        )
        if not metadata_matches:
            return False
        for key in ("test_acc", "test_nll", "test_ece"):
            float(row[key])
        if target.family != "teacher":
            for key in (
                "test_ts_kl",
                "test_fisher_mismatch",
                "test_energy_mismatch",
            ):
                float(row[key])
        return True
    except (KeyError, TypeError, ValueError):
        return False


def numeric_mean_std(
    rows: list[dict[str, object]], key: str
) -> tuple[object, object]:
    values = [
        float(row[key])
        for row in rows
        if row.get(key) not in (None, "")
    ]
    if not values:
        return "", ""
    if len(values) == 1:
        return values[0], 0.0
    return mean(values), stdev(values)


def build_summary(rows: list[dict[str, object]]) -> list[dict[str, object]]:
    grouped: dict[tuple[str, str, str], list[dict[str, object]]] = {}
    for row in rows:
        key = (
            str(row["selection_status"]),
            str(row["family"]),
            str(row["variant"]),
        )
        grouped.setdefault(key, []).append(row)

    family_order = {"teacher": 0, "baseline": 1, "alternating": 2}
    summaries: list[dict[str, object]] = []
    ordered_groups = sorted(
        grouped.values(),
        key=lambda items: (
            family_order.get(str(items[0]["family"]), 99),
            str(items[0]["selection_status"]),
            str(items[0]["variant"]),
        ),
    )
    metric_keys = (
        "test_acc",
        "test_nll",
        "test_ece",
        "test_ts_kl",
        "test_fisher_mismatch",
        "test_energy_mismatch",
    )
    for variant_rows in ordered_groups:
        first = variant_rows[0]
        summary: dict[str, object] = {
            "group": first["group"],
            "dataset": first["dataset"],
            "architecture": first["architecture"],
            "family": first["family"],
            "selection_status": first["selection_status"],
            "method": first["method"],
            "variant": first["variant"],
            "rho": first["rho"],
            "n_seeds": len(variant_rows),
            "ece_bins": first["ece_bins"],
            "diagnostic_temperature": DIAGNOSTIC_TEMPERATURE,
        }
        for metric_key in metric_keys:
            metric_mean, metric_std = numeric_mean_std(variant_rows, metric_key)
            summary[f"{metric_key}_mean"] = metric_mean
            summary[f"{metric_key}_std"] = metric_std
        summaries.append(summary)
    return summaries


def print_inventory(targets: list[EvaluationTarget], warnings: list[str]) -> None:
    print("\nFinal global test re-evaluation inventory")
    print("=" * 88)
    for group_key in DEFAULT_GROUPS:
        group_targets = [target for target in targets if target.group.key == group_key]
        if not group_targets:
            continue
        variants: dict[tuple[str, str], list[EvaluationTarget]] = {}
        for target in group_targets:
            variants.setdefault(
                (target.selection_status, target.variant), []
            ).append(target)
        print(f"{GROUPS[group_key].label}: {len(group_targets)} checkpoint(s)")
        for (status, variant), variant_targets in sorted(variants.items()):
            seeds = [
                target.seed for target in variant_targets if target.seed is not None
            ]
            seed_text = "teacher" if not seeds else "seeds " + ",".join(map(str, seeds))
            print(f"  - [{status}] {variant}: {seed_text}")
    if warnings:
        print("\nInventory warnings")
        print("-" * 88)
        for warning in warnings:
            print(f"  - {warning}")


def format_mean_std(row: dict[str, object], metric: str) -> str:
    metric_mean = row.get(f"{metric}_mean", "")
    metric_std = row.get(f"{metric}_std", "")
    if metric_mean in (None, ""):
        return "n/a"
    return f"{float(metric_mean):.4f} +/- {float(metric_std):.4f}"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Recompute global accuracy, NLL, ECE, teacher-student KL, output-"
            "Fisher mismatch, and energy mismatch for final checkpoints."
        )
    )
    parser.add_argument(
        "--groups",
        nargs="+",
        choices=("all",) + DEFAULT_GROUPS,
        default=("all",),
        help="Experiment groups to evaluate (default: all).",
    )
    parser.add_argument(
        "--seeds",
        nargs="+",
        type=int,
        default=DEFAULT_SEEDS,
        help="Required final seeds (default: 42 43 44 45 46).",
    )
    parser.add_argument(
        "--checkpoint-root",
        default="checkpoints",
        help="Checkpoint root relative to the repository (default: checkpoints).",
    )
    parser.add_argument(
        "--results-root",
        default="results",
        help="Original results root relative to the repository (default: results).",
    )
    parser.add_argument(
        "--output-dir",
        default="results/global_evaluation_final",
        help=(
            "Separate output directory (default: "
            "results/global_evaluation_final)."
        ),
    )
    parser.add_argument(
        "--data-root",
        default=None,
        help="Override the data root from the YAML configs.",
    )
    parser.add_argument("--batch-size", type=int, default=None)
    parser.add_argument("--num-workers", type=int, default=None)
    parser.add_argument("--ece-bins", type=int, default=15)
    parser.add_argument(
        "--device",
        choices=("auto", "cpu", "cuda"),
        default="auto",
    )
    parser.add_argument(
        "--include-baselines",
        action=argparse.BooleanOptionalAction,
        default=True,
    )
    parser.add_argument(
        "--include-alternating",
        action=argparse.BooleanOptionalAction,
        default=True,
    )
    parser.add_argument(
        "--include-teachers",
        action=argparse.BooleanOptionalAction,
        default=True,
    )
    parser.add_argument(
        "--alternating-scope",
        choices=("selected", "all-complete"),
        default="selected",
        help=(
            "Use only validation-selected final variants (default), or also "
            "evaluate other complete rho variants as sensitivity rows."
        ),
    )
    parser.add_argument(
        "--allow-incomplete",
        action="store_true",
        help="Evaluate partial seed sets too; by default they are only reported.",
    )
    parser.add_argument(
        "--rerun",
        action="store_true",
        help="Recompute rows already current in the final global CSVs.",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Print the exact checkpoint inventory without loading models or data.",
    )
    args = parser.parse_args()

    if "all" in args.groups and len(args.groups) != 1:
        parser.error("Use either --groups all or an explicit group list, not both")
    if len(set(args.seeds)) != len(args.seeds):
        parser.error("Seeds must not contain duplicates")
    if args.batch_size is not None and args.batch_size <= 0:
        parser.error("--batch-size must be positive")
    if args.num_workers is not None and args.num_workers < 0:
        parser.error("--num-workers must be nonnegative")
    if args.ece_bins <= 0:
        parser.error("--ece-bins must be positive")
    return args


def main() -> None:
    args = parse_args()
    group_keys = (
        DEFAULT_GROUPS if tuple(args.groups) == ("all",) else tuple(args.groups)
    )
    specs = [GROUPS[key] for key in group_keys]
    checkpoint_root = repository_path(args.checkpoint_root)
    results_root = repository_path(args.results_root)
    output_dir = repository_path(args.output_dir)
    seeds = tuple(args.seeds)

    targets, warnings = discover_targets(
        specs=specs,
        checkpoint_root=checkpoint_root,
        results_root=results_root,
        seeds=seeds,
        include_baselines=args.include_baselines,
        include_alternating=args.include_alternating,
        include_teachers=args.include_teachers,
        alternating_scope=args.alternating_scope,
        allow_incomplete=args.allow_incomplete,
    )
    print_inventory(targets, warnings)
    if args.dry_run:
        print(f"\nDry run only: {len(targets)} checkpoint(s) would be evaluated.")
        return
    if not targets:
        raise SystemExit(
            "No eligible checkpoints found. Run with --dry-run to inspect paths."
        )

    if args.device == "auto":
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    else:
        device = torch.device(args.device)
    if device.type == "cuda" and not torch.cuda.is_available():
        raise RuntimeError("CUDA was requested but torch.cuda.is_available() is false")
    print(f"\nEvaluation device: {device}")
    print(
        "Protocol: locked variants; global sample weighting; "
        f"ECE={args.ece_bins} bins; diagnostic T={DIAGNOSTIC_TEMPERATURE:g}"
    )

    loaders: dict[str, object] = {}
    teacher_caches: dict[str, TeacherCache] = {}
    rows_by_group: dict[str, list[dict[str, object]]] = {}
    existing_by_group: dict[str, dict[str, dict[str, str]]] = {}
    target_keys_by_group = {
        spec.key: {
            display_path(target.checkpoint_path)
            for target in targets
            if target.group.key == spec.key
        }
        for spec in specs
    }

    for spec in specs:
        output_path = output_dir / f"{spec.key}_global_test_metrics.csv"
        existing_rows = [
            row
            for row in read_csv(output_path)
            if row.get("checkpoint_path") in target_keys_by_group[spec.key]
        ]
        existing_by_group[spec.key] = {
            row["checkpoint_path"]: row
            for row in existing_rows
            if row.get("checkpoint_path")
        }
        rows_by_group[spec.key] = list(existing_rows)

    for index, target in enumerate(targets, start=1):
        checkpoint_key = display_path(target.checkpoint_path)
        existing = existing_by_group[target.group.key].get(checkpoint_key)
        if existing is not None and not args.rerun and result_is_current(
            row=existing,
            target=target,
            checkpoint_root=checkpoint_root,
            n_bins=args.ece_bins,
        ):
            print(f"[{index}/{len(targets)}] SKIP current: {checkpoint_key}")
            continue

        spec = target.group
        if spec.key not in loaders:
            data_cfg = load_yaml(repository_path(spec.student_config))["data"]
            data_root = (
                repository_path(args.data_root)
                if args.data_root is not None
                else repository_path(data_cfg["root"])
            )
            batch_size = args.batch_size or int(data_cfg["batch_size"])
            num_workers = (
                args.num_workers
                if args.num_workers is not None
                else int(data_cfg["num_workers"])
            )
            loaders[spec.key] = get_image_test_loader(
                dataset_name=spec.dataset,
                root=str(data_root),
                batch_size=batch_size,
                num_workers=num_workers,
            )

        if spec.key not in teacher_caches:
            print(
                f"[{index}/{len(targets)}] CACHE teacher: "
                f"{display_path(checkpoint_root / spec.teacher_checkpoint)}"
            )
            teacher_caches[spec.key] = build_teacher_cache(
                spec=spec,
                loader=loaders[spec.key],
                checkpoint_root=checkpoint_root,
                device=device,
                n_bins=args.ece_bins,
            )

        print(f"[{index}/{len(targets)}] EVAL: {checkpoint_key}")
        if target.family == "teacher":
            metrics = dict(teacher_caches[spec.key].metrics)
        else:
            metrics = evaluate_student_checkpoint(
                target=target,
                loader=loaders[spec.key],
                teacher_cache=teacher_caches[spec.key],
                device=device,
                n_bins=args.ece_bins,
            )
        row = make_result_row(
            target=target,
            metrics=metrics,
            n_bins=args.ece_bins,
            checkpoint_root=checkpoint_root,
        )

        group_rows = rows_by_group[target.group.key]
        group_rows[:] = [
            old_row
            for old_row in group_rows
            if old_row.get("checkpoint_path") != checkpoint_key
        ]
        group_rows.append(row)
        group_rows.sort(
            key=lambda item: (
                str(item.get("family", "")),
                str(item.get("selection_status", "")),
                str(item.get("variant", "")),
                -1 if item.get("seed", "") == "" else int(item["seed"]),
            )
        )
        output_path = output_dir / f"{target.group.key}_global_test_metrics.csv"
        write_csv_atomic(output_path, PER_CHECKPOINT_FIELDS, group_rows)
        existing_by_group[target.group.key][checkpoint_key] = {
            key: str(value) for key, value in row.items()
        }

        message = (
            f"    acc={float(metrics['test_acc']):.6f} | "
            f"nll={float(metrics['test_nll']):.6f} | "
            f"global ECE={float(metrics['test_ece']):.6f}"
        )
        if target.family != "teacher":
            message += (
                f" | TS-KL={float(metrics['test_ts_kl']):.6f} | "
                f"Fisher={float(metrics['test_fisher_mismatch']):.6f} | "
                f"energy={float(metrics['test_energy_mismatch']):.6f}"
            )
        print(message)

    print("\nFinal global test summaries")
    print("=" * 88)
    for spec in specs:
        group_rows = rows_by_group[spec.key]
        if not group_rows:
            continue
        summary = build_summary(group_rows)
        metrics_path = output_dir / f"{spec.key}_global_test_metrics.csv"
        summary_path = output_dir / f"{spec.key}_global_test_summary.csv"
        write_csv_atomic(metrics_path, PER_CHECKPOINT_FIELDS, group_rows)
        write_csv_atomic(summary_path, SUMMARY_FIELDS, summary)
        print(f"{spec.label}:")
        for row in summary:
            print(
                f"  [{row['selection_status']}] {row['method']} | "
                f"n={row['n_seeds']} | "
                f"acc={format_mean_std(row, 'test_acc')} | "
                f"nll={format_mean_std(row, 'test_nll')} | "
                f"ECE={format_mean_std(row, 'test_ece')} | "
                f"TS-KL={format_mean_std(row, 'test_ts_kl')} | "
                f"Fisher={format_mean_std(row, 'test_fisher_mismatch')} | "
                f"energy={format_mean_std(row, 'test_energy_mismatch')}"
            )
        print(f"  saved: {display_path(metrics_path)}")
        print(f"  saved: {display_path(summary_path)}")


if __name__ == "__main__":
    main()
