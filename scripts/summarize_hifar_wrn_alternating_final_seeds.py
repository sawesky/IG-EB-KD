#!/usr/bin/env python3
"""Summarize the locked CIFAR-100 WRN alternating final-seed comparison."""

import argparse
import csv
import math
from pathlib import Path
from statistics import mean, stdev


DEFAULT_SEEDS = [42, 43, 44, 45, 46]
DEFAULT_HEAD_LR = 0.05
FINAL_LR_MILESTONE = 120
RESULT_DIR = Path("results/head_metric/alternating/seeds")
OUT_PATH = Path(
    "results/head_metric/alternating/"
    "hifar_wrn_alternating_final_seeds_summary.csv"
)
PER_SEED_OUT_PATH = Path(
    "results/head_metric/alternating/"
    "hifar_wrn_alternating_final_seeds_per_seed.csv"
)


def value_to_name(value):
    text = f"{value:.8f}".rstrip("0").rstrip(".")
    return text.replace(".", "p")


def get_float(row, key, default=0.0):
    value = row.get(key, "")
    if value in ("", None):
        return default
    return float(value)


def summarize_file(path, expected_head_lr):
    with path.open("r", newline="") as handle:
        rows = list(csv.DictReader(handle))
    val_rows = [row for row in rows if row.get("phase") == "val"]
    test_rows = [row for row in rows if row.get("phase") == "test"]
    if not val_rows:
        raise RuntimeError(f"No validation rows found in {path}")
    if not test_rows:
        raise RuntimeError(f"No test row found in {path}")

    try:
        observed_head_lrs = {float(row["head_lr"]) for row in val_rows}
    except (KeyError, TypeError, ValueError) as error:
        raise RuntimeError(f"Invalid head LR provenance in {path}") from error
    if len(observed_head_lrs) != 1:
        raise RuntimeError(
            f"Expected one constant head LR in {path}, found "
            f"{sorted(observed_head_lrs)}"
        )
    observed_head_lr = observed_head_lrs.pop()
    if not math.isclose(
        observed_head_lr,
        expected_head_lr,
        rel_tol=1e-9,
        abs_tol=1e-12,
    ):
        raise RuntimeError(
            f"Expected constant head LR {expected_head_lr:g}, but {path} "
            f"records {observed_head_lr!r}"
        )

    best = min(val_rows, key=lambda row: get_float(row, "val_nll"))
    test = test_rows[-1]
    best_epoch = int(float(best["epoch"]))
    recorded_best_epoch = int(float(test.get("best_epoch", best_epoch)))
    if recorded_best_epoch != best_epoch:
        raise RuntimeError(
            f"Best-epoch mismatch in {path}: validation rows give {best_epoch}, "
            f"test row records {recorded_best_epoch}"
        )
    last_val_epoch = int(float(val_rows[-1]["epoch"]))

    return {
        "head_lr": observed_head_lr,
        "best_epoch": best_epoch,
        "last_val_epoch": last_val_epoch,
        "epochs_after_best": last_val_epoch - best_epoch,
        "epochs_after_final_lr_milestone": max(
            0,
            last_val_epoch - FINAL_LR_MILESTONE,
        ),
        "test_acc": get_float(test, "test_acc"),
        "test_nll": get_float(test, "test_nll"),
        "test_ece": get_float(test, "test_ece"),
        "test_ts_kl": get_float(test, "test_teacher_student_kl"),
        "test_fisher_mismatch": get_float(test, "test_fisher_mismatch"),
        "test_energy_mismatch": get_float(test, "test_energy_mismatch"),
    }


def mean_std(values):
    if len(values) == 1:
        return mean(values), 0.0
    return mean(values), stdev(values)


def make_runs(student_rho, teacher_rho):
    return [
        {
            "method": "WRN-16-2 KD + Euclidean head [alternating]",
            "rho": 0.0,
            "base_name": "hifar_wrn16_2_kd_head_euclidean_alternating",
        },
        {
            "method": "WRN-16-2 KD + Student-Fisher [alternating]",
            "rho": student_rho,
            "base_name": (
                "hifar_wrn16_2_kd_head_student_fisher_"
                f"rho{value_to_name(student_rho)}_alternating"
            ),
        },
        {
            "method": "WRN-16-2 KD + Teacher-induced Fisher [alternating]",
            "rho": teacher_rho,
            "base_name": (
                "hifar_wrn16_2_kd_head_teacher_fisher_"
                f"rho{value_to_name(teacher_rho)}_alternating"
            ),
        },
    ]


def main():
    parser = argparse.ArgumentParser(
        description=(
            "Aggregate the locked CIFAR-100 WRN alternating five-seed "
            "comparison and retain per-seed rows."
        )
    )
    parser.add_argument("--student-rho", type=float, required=True)
    parser.add_argument("--teacher-rho", type=float, required=True)
    parser.add_argument(
        "--head-lr",
        type=float,
        default=DEFAULT_HEAD_LR,
        help="Expected constant manual head LR (default: 0.05).",
    )
    parser.add_argument(
        "--seeds",
        nargs="+",
        type=int,
        default=DEFAULT_SEEDS,
    )
    parser.add_argument(
        "--allow-missing",
        action="store_true",
        help="Write a provisional summary even if expected runs are missing.",
    )
    args = parser.parse_args()

    if args.student_rho <= 0.0 or args.teacher_rho <= 0.0:
        raise ValueError("Selected Fisher rho values must be positive")
    if args.head_lr <= 0.0:
        raise ValueError("Head LR must be positive")
    if len(set(args.seeds)) != len(args.seeds):
        raise ValueError("Seeds must not contain duplicates")

    summary_rows = []
    per_seed_rows = []
    missing = []
    for run in make_runs(args.student_rho, args.teacher_rho):
        seed_rows = []
        for seed in args.seeds:
            path = RESULT_DIR / f"{run['base_name']}_seed{seed}.csv"
            if not path.exists():
                missing.append(str(path))
                continue
            try:
                row = summarize_file(path, args.head_lr)
            except RuntimeError as error:
                missing.append(str(error))
                continue
            row.update(
                {
                    "method": run["method"],
                    "rho": run["rho"],
                    "seed": seed,
                    "metrics_path": str(path),
                }
            )
            seed_rows.append(row)
            per_seed_rows.append(row)

        if not seed_rows:
            continue

        out = {
            "method": run["method"],
            "rho": run["rho"],
            "head_lr": args.head_lr,
            "n_seeds": len(seed_rows),
        }
        for metric in (
            "best_epoch",
            "last_val_epoch",
            "epochs_after_best",
            "epochs_after_final_lr_milestone",
            "test_acc",
            "test_nll",
            "test_ece",
            "test_ts_kl",
            "test_fisher_mismatch",
            "test_energy_mismatch",
        ):
            values = [row[metric] for row in seed_rows]
            metric_mean, metric_std = mean_std(values)
            out[f"{metric}_mean"] = metric_mean
            out[f"{metric}_std"] = metric_std
        summary_rows.append(out)

    if missing and not args.allow_missing:
        print("Final summary not written because expected runs are missing:")
        for item in missing:
            print(f"  - {item}")
        raise SystemExit(2)

    summary_fieldnames = [
        "method",
        "rho",
        "head_lr",
        "n_seeds",
        "best_epoch_mean",
        "best_epoch_std",
        "last_val_epoch_mean",
        "last_val_epoch_std",
        "epochs_after_best_mean",
        "epochs_after_best_std",
        "epochs_after_final_lr_milestone_mean",
        "epochs_after_final_lr_milestone_std",
        "test_acc_mean",
        "test_acc_std",
        "test_nll_mean",
        "test_nll_std",
        "test_ece_mean",
        "test_ece_std",
        "test_ts_kl_mean",
        "test_ts_kl_std",
        "test_fisher_mismatch_mean",
        "test_fisher_mismatch_std",
        "test_energy_mismatch_mean",
        "test_energy_mismatch_std",
    ]
    per_seed_fieldnames = [
        "method",
        "rho",
        "head_lr",
        "seed",
        "metrics_path",
        "best_epoch",
        "last_val_epoch",
        "epochs_after_best",
        "epochs_after_final_lr_milestone",
        "test_acc",
        "test_nll",
        "test_ece",
        "test_ts_kl",
        "test_fisher_mismatch",
        "test_energy_mismatch",
    ]

    OUT_PATH.parent.mkdir(parents=True, exist_ok=True)
    with OUT_PATH.open("w", newline="") as handle:
        writer = csv.DictWriter(handle, fieldnames=summary_fieldnames)
        writer.writeheader()
        writer.writerows(summary_rows)
    with PER_SEED_OUT_PATH.open("w", newline="") as handle:
        writer = csv.DictWriter(handle, fieldnames=per_seed_fieldnames)
        writer.writeheader()
        writer.writerows(per_seed_rows)

    print("\nCIFAR-100 WRN alternating final five-seed summary")
    print(
        "method | rho | head LR | n | best/last epoch | "
        "test acc mean +/- std | test NLL | test ECE"
    )
    print("-" * 145)
    for row in summary_rows:
        print(
            f"{row['method']} | "
            f"{float(row['rho']):g} | "
            f"{float(row['head_lr']):g} | "
            f"{row['n_seeds']} | "
            f"{float(row['best_epoch_mean']):.1f}/"
            f"{float(row['last_val_epoch_mean']):.1f} | "
            f"{float(row['test_acc_mean']):.4f} +/- "
            f"{float(row['test_acc_std']):.4f} | "
            f"{float(row['test_nll_mean']):.4f} +/- "
            f"{float(row['test_nll_std']):.4f} | "
            f"{float(row['test_ece_mean']):.4f} +/- "
            f"{float(row['test_ece_std']):.4f}"
        )

    if missing:
        print(f"\nWARNING: provisional summary; {len(missing)} run(s) missing.")
    print(f"\nsaved aggregate summary: {OUT_PATH}")
    print(f"saved per-seed summary: {PER_SEED_OUT_PATH}")


if __name__ == "__main__":
    main()
