...
def summarize_file(path):
    ...
    observed_head_lrs = {
        get_float(row, "head_lr", default=float("nan")) for row in val_rows
    }
    ...
    if not math.isclose(
        ...
        raise RuntimeError(
            f"Expected constant head LR {HEAD_LR:g}, but {path} "
    ...
    return [
        {
            ...
            "rho": 0.0,
            "base_name": "hifar_resnet20_kd_head_euclidean_alternating",
        },
        {
...
def main():
    ...
    print("\nCIFAR-100 ResNet alternating final five-seed summary")
    print(
        "method | rho | head LR | n | test acc mean +/- std | "
        "test NLL | test ECE"
    )
    print("-" * 115)
