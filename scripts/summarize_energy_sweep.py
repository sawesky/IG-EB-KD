...
PATTERN = "mnist_student_kd_energy_b*.csv"
OUT_PATH = RESULTS_DIR / "energy_sweep_summary.csv"


def parse_beta(path):
    # Example:
    # mnist_student_kd_energy_b0p003.csv -> 0.003
    stem = path.stem
    beta_part = stem.split("_b")[-1]
    return float(beta_part.replace("p", "."))
...
def save_summary(rows):
    ...
    print(f"\nsaved Energy sweep summary: {OUT_PATH}")


def main():
    files = sorted(RESULTS_DIR.glob(PATTERN), key=parse_beta)
    ...
    if len(rows) == 0:
        print("No valid Energy sweep files found.")
        return
    ...
    best_energy = min(rows, key=lambda row: row["best_val_energy_mismatch"])

    print(
        "\nBest by validation NLL: "
        f"beta={best_nll['beta']}, "
