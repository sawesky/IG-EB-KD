...
def print_summary(summary_rows):
    ...
    print("\nHIFAR / CIFAR-100 WRN KD SWEEP SUMMARY")
    print(header)
    print("-" * len(header))
...
def save_summary(summary_rows):
    out_path = Path("results/hifar_wrn_kd_sweep_summary.csv")
    ...
    print(f"\nsaved HIFAR WRN KD sweep summary: {out_path}")
...
def main():
    ...
    for T in TEMPERATURES:
        for lam in LAMBDAS:
            ...
            name = f"hifar_wrn16_2_kd_T{value_to_name(T)}_l{value_to_name(lam)}"
            ...
            print("\n" + "=" * 80)
            print(f"Running HIFAR WRN KD sweep: T={T}, lambda={lam}")
            print("=" * 80)
