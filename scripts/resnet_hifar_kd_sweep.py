...
def print_summary(summary_rows):
    ...
    print("\nHIFAR / CIFAR-100 RESNET KD SWEEP SUMMARY")
    print(header)
    print("-" * len(header))
...
def save_summary(summary_rows):
    out_path = Path("results/hifar_resnet_kd_sweep_summary.csv")
...
def main():
    ...
    for T in TEMPERATURES:
        for lam in LAMBDAS:
            ...
            name = f"hifar_resnet20_kd_T{value_to_name(T)}_l{value_to_name(lam)}"
            ...
            print("\n" + "=" * 80)
            print(f"Running HIFAR ResNet KD sweep: T={T}, lambda={lam}")
            print("=" * 80)
