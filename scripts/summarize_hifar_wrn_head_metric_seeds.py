...
RESULT_DIR = Path("results/head_metric/seeds")
OUT_PATH = Path("results/head_metric/hifar_wrn_head_metric_seeds_summary.csv")
...
def make_runs(student_rho, teacher_rho):
    return [
        {
            "method": "WRN-16-2 KD + Euclidean head",
            "rho": 0.0,
            "base_name": "hifar_wrn16_2_kd_head_euclidean",
        },
        {
            "method": "WRN-16-2 KD + Student-Fisher",
            "rho": student_rho,
            "base_name": (
        ...
        {
            "method": "WRN-16-2 KD + Teacher-induced Fisher",
            "rho": teacher_rho,
            "base_name": (
...
def main():
    ...
    print("\nCIFAR-100 WRN head-metric multi-seed summary")
    print("method | rho | n | acc mean_std | nll mean_std | ece mean_std | TS-KL mean_std")
    print("-" * 120)
    ...
    for row in summary_rows:
        print(
            ...
            f"{float(row['test_ts_kl_mean']):.4f}_{float(row['test_ts_kl_std']):.4f}"
        )
