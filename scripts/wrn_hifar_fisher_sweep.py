...
BASE_CONFIG = "configs/hifar_wrn_student_kd_fisher.yaml"
...
OUT_PATH = Path("results/hifar_wrn_fisher_sweep_summary.csv")
...
def summarize_run(alpha, metrics_path):
    ...
    best_val_row = min(
        val_rows,
        key=lambda row: get_float(row, "val_nll"),
    )
    ...
    return {
        ...
        "best_val_ts_kl": get_float(
            best_val_row,
            "val_teacher_student_kl",
        ),
        "best_val_fisher_mismatch": get_float(
            best_val_row,
            "val_fisher_mismatch",
        ),
        "best_val_energy_mismatch": get_float(
            best_val_row,
            "val_energy_mismatch",
        ),
        ...
        "test_ts_kl": get_float(
            test_row,
            "test_teacher_student_kl",
        ),
        "test_fisher_mismatch": get_float(
            test_row,
            "test_fisher_mismatch",
        ),
        "test_energy_mismatch": get_float(
            test_row,
...
def print_summary(rows):
    ...
    print("\nHIFAR / CIFAR-100 WRN FISHER SWEEP SUMMARY")
    header = " | ".join(
...
def save_summary(rows):
    ...
    print(
        f"\nsaved HIFAR WRN Fisher sweep summary: "
        f"{OUT_PATH}"
    )
...
def main():
    ...
    for alpha in ALPHAS:
        ...
        name = (
            f"hifar_wrn16_2_kd_fisher_"
            f"T{value_to_name(BEST_KD_T)}_"
            f"l{value_to_name(BEST_KD_LAMBDA)}_"
            f"a{value_to_name(alpha)}"
        )

        cfg["experiment_name"] = name

        # Fixed best KD baseline from HIFAR WRN KD sweep.
        ...
        print("\n" + "=" * 80)
        print(
            f"Running HIFAR WRN Fisher sweep: "
            f"T={BEST_KD_T}, "
            f"lambda={BEST_KD_LAMBDA}, "
            f"alpha={alpha}"
        )
        print("=" * 80)
    ...
    summary_rows = sorted(
        summary_rows,
        key=lambda row: row["best_val_nll"],
    )
    ...
    best_fisher = min(
        summary_rows,
        key=lambda row: row["best_val_fisher_mismatch"],
    )

    print(
        "\nBest by validation NLL: "
        f"alpha={best_nll['alpha']:g}, "
    ...
    print(
        ...
        f"alpha={best_fisher['alpha']:g}, "
        f"val_fisher="
        f"{best_fisher['best_val_fisher_mismatch']:.6f}, "
        f"test_fisher="
        f"{best_fisher['test_fisher_mismatch']:.6f}, "
        f"test_nll={best_fisher['test_nll']:.6f}, "
        f"test_acc={best_fisher['test_acc']:.6f}"
    )
