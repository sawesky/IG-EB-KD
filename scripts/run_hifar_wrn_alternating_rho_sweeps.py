#!/usr/bin/env python3
"""Run the CIFAR-100 WRN seed-42 alternating Fisher rho sweeps."""
...
def make_run(metric, rho, head_lr):
    if metric == "student":
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Run seed-42 validation sweeps for CIFAR-100 WRN alternating "
            "Student-Fisher and Teacher-induced Fisher head updates."
        )
    )
    parser.add_argument(
        "--metric",
    ...
    for metric in metrics:
        for rho in args.rhos:
            run = make_run(metric, rho, args.head_lr)
            ...
            print("\n" + "=" * 80)
            print(
                f"CIFAR-100 WRN alternating {run['label']} | "
                f"rho={rho:g} | constant head LR={args.head_lr:g} | "
    ...
    print(
        ...
        "python scripts/summarize_hifar_wrn_alternating_rho_sweeps.py "
