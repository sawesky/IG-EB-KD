...
BASE_CONFIG = "configs/cifar_student_kd_fisher_energy.yaml"
...
def save_summary(summary_rows):
    out_path = Path("results/cifar_combined_sweep_summary.csv")
    ...
    print(f"\nsaved CIFAR combined sweep summary: {out_path}")
...
def main():
    ...
    for alpha in ALPHAS:
        for beta in BETAS:
            ...
            name = (
                f"cifar_student_kd_fisher_energy_"
                f"a{value_to_name(alpha)}_b{value_to_name(beta)}"
            )

            cfg["experiment_name"] = name

            cfg["kd"]["temperature"] = KD_TEMPERATURE
            cfg["kd"]["lambda_kd"] = KD_LAMBDA
            ...
            print("\n" + "=" * 80)
            print(f"Running CIFAR combined sweep: alpha={alpha}, beta={beta}")
            print("=" * 80)
