#!/usr/bin/env python3
"""Run the final CIFAR-100 WRN alternating five-seed comparison safely."""
...
BASE_CONFIG = "configs/hifar_wrn_student_kd.yaml"
...
def metrics_status(path, expected_head_lr):
...
def make_runs(student_rho, teacher_rho, head_lr):
    return {
        "euclidean": {
            "label": f"Euclidean head (constant LR={head_lr:g})",
            "name": "hifar_wrn16_2_kd_head_euclidean_alternating",
            "head_update": {
                "mode": "euclidean",
                "scheme": "alternating",
                "rho": 0.0,
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Run the final CIFAR-100 WRN alternating Euclidean, "
            "Student-Fisher, and Teacher-induced Fisher comparison."
        )
    )
    parser.add_argument(
        "--student-rho",
    ...
    parser.add_argument(
        ...
        help="Locked constant manual head LR (default: 0.05).",
    )
    parser.add_argument(
        "--seeds",
        nargs="+",
    ...
    parser.add_argument(
        ...
        default=0.0,
        help=(
            "Idle time before each training run after the first scheduled run "
            "(default: 0)."
        ),
    )
    parser.add_argument(
    ...
    all_runs = make_runs(args.student_rho, args.teacher_rho, args.head_lr)
    ...
    for seed in args.seeds:
        for run in runs:
            ...
            print("\n" + "=" * 80)
            print(f"CIFAR-100 WRN alternating {run['label']} | seed={seed}")

            status = metrics_status(metrics_path, args.head_lr)
            ...
            if status == "incompatible":
                print(
                    ...
                    f"constant head LR={args.head_lr:g}: {metrics_path}"
                )
                replaced += 1
    ...
    print(
        "Summarize the final seeds with:\n"
        "python scripts/summarize_hifar_wrn_alternating_final_seeds.py "
        f"--head-lr {args.head_lr:g} "
