...
BASE_CONFIG = "configs/cifar_student_kd_fisher.yaml"
ALPHAS = [0.05, 0.1, 0.5, 1.0, 5.0]
OUT_PATH = Path("results/cifar_fisher_sweep_summary.csv")
...
def save_summary(rows):
    ...
    print(f"\nsaved CIFAR Fisher sweep summary: {OUT_PATH}")
...
def main():
    ...
    for alpha in ALPHAS:
        ...
        name = f"cifar_student_kd_fisher_a{value_to_name(alpha)}"
        cfg["experiment_name"] = name
        cfg["kd"]["temperature"] = 4.0
        cfg["kd"]["lambda_kd"] = 0.5
        ...
        print("\n" + "=" * 80)
        print(f"Running CIFAR Fisher sweep: alpha={alpha}")
        print("=" * 80)
