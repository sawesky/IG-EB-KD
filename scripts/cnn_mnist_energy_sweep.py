...
OUT_PATH = Path("results/mnist_energy_sweep_summary.csv")
...
def summarize_run(beta, metrics_path):
    ...
    return {
        ...
        "best_val_ts_kl": get_float(
            best_val_row, "val_teacher_student_kl"
        ),
        ...
        "test_ts_kl": get_float(
            test_row, "test_teacher_student_kl"
        ),
...
def print_summary(rows):
    ...
    print("\nMNIST LENET ENERGY SWEEP SUMMARY")
...
def save_summary(rows):
    ...
    print(f"\nsaved MNIST Energy sweep summary: {OUT_PATH}")
...
def main():
    ...
    for beta in BETAS:
        ...
        print("\n" + "=" * 80)
        print(
            f"Running MNIST LeNet Energy sweep: "
            f"T={BEST_KD_T}, "
        ...
        summary_rows.append(
            summarize_run(beta, cfg["save"]["metrics_path"])
        )
    ...
    best_energy = min(
        summary_rows,
        key=lambda row: row["best_val_energy_mismatch"],
    )
