...
OUT_PATH = Path(
    ...
    "cifar_wrn_alternating_rho_sweeps_validation_summary.csv"
)
...
def metrics_path(metric, rho, seed):
    if metric == "student":
        stem = "cifar_wrn16_2_kd_head_student_fisher"
    elif metric == "teacher":
        stem = "cifar_wrn16_2_kd_head_teacher_fisher"
    else:
...
def print_table(metric, rows, tie_tolerance):
    ...
    print(f"\nCIFAR-10 WRN ALTERNATING {label} RHO SWEEP")
    print(
